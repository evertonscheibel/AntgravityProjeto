...
export const getDashboardKPIs = async (req, res, next) => {
    try {
        ...
        const [
            ...
            Boleto.countDocuments({ status: 'pendente' }),
            Boleto.countDocuments({ status: 'atrasado' }),
            SlaughterClosure.aggregate([
                {
                    $facet: {
                        todayPlanned: [
                            { 
                        ...
                        closedThisMonth: [
                            ...
                            { $group: { _id: null, total: { $sum: '$totalCattle' } } }
                        ]
                    }
                }
            ]),
            SlaughterClosure.aggregate([
                ...
                { $unwind: '$lines' },
                {
                    $group: {
                        ...
                        boi: { $sum: '$lines.boi' },
                        vaca: { $sum: '$lines.vaca' },
                        novilha: { $sum: '$lines.novilha' },
                        bubalino: { $sum: '$lines.bubalino' },
                        touro: { $sum: '$lines.touro' }
                    }
                }
            ])
        ]);
        ...
        res.json({
            ...
            data: {
                ...
                slaughter: {
                    todayPlanned: slaughterGeneralStats[0].todayPlanned[0]?.totalCattle || 0,
                    pendingClosures: slaughterGeneralStats[0].pendingClosures[0]?.count || 0,
                    closedThisMonth: slaughterGeneralStats[0].closedThisMonth[0]?.total || 0,
                    categoriesData: slaughterByMonth[0] ? [
                        { name: 'Boi', value: slaughterByMonth[0].boi || 0 },
                        { name: 'Vaca', value: slaughterByMonth[0].vaca || 0 },
                        { name: 'Novilha', value: slaughterByMonth[0].novilha || 0 },
                        { name: 'Bubalino', value: slaughterByMonth[0].bubalino || 0 },
                        { name: 'Touro', value: slaughterByMonth[0].touro || 0 }
                    ] : []
                }
            }
        });
...
export const getAlerts = async (req, res, next) => {
    try {
        ...
        const [expiringCerts, expiringWarranties, overdueMaintenance, boletosOverdue] = await Promise.all([
            Certificate.find({
                expirationDate: { $lte: futureDate, $gte: new Date() },
                status: 'ativo'
