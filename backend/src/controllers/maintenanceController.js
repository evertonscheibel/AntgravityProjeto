...
export const updateMaintenance = async (req, res, next) => {
    try {
        ...
        // Se manutenção foi concluída, chamar lógica de integração ERP
        if (maintenance.status === 'concluida' && oldMaintenance.status !== 'concluida') {
            // Reaproveitar lógica de finalização se necessário ou deixar o fluxo manual
            // Para automação completa, poderíamos chamar uma função interna aqui
        }
...
// @desc    Finalizar manutenção (ERP Integration)
// @route   POST /api/maintenances/:id/complete
// @access  Private
export const completeMaintenance = async (req, res, next) => {
    try {
        ...
        if (maintenance.status === 'concluida') {
            return res.status(400).json({ success: false, message: 'Manutenção já está concluída' });
        }

        // 1. Atualizar Status da Manutenção e Datas
        maintenance.status = 'concluida';
        maintenance.endDate = req.body.endDate || new Date();
        await maintenance.save();

        // 2. Almoxarifado: Abater saldo de insumos/peças
        if (maintenance.parts && maintenance.parts.length > 0) {
            for (const part of maintenance.parts) {
                // Tenta encontrar o insumo pelo nome na mesma fazenda do ativo
                const insumo = await Insumo.findOne({
                    nome: { $regex: new RegExp(`^${part.name}$`, 'i') },
                    fazenda_id: maintenance.asset.fazenda_id
                });

                if (insumo) {
                    insumo.quantidade_atual -= part.quantity;
                    await insumo.save();
                    console.log(`[ERP] Estoque abatido: ${part.name} (-${part.quantity})`);
                } else {
                    console.warn(`[ERP] Insumo "${part.name}" não encontrado para baixa automática.`);
                }
            }
        }

        // 3. Custos: Gerar Lançamento de Custo
        // Busca um Centro de Custo padrão para a fazenda se não houver um específico vinculado
        const centroCusto = await CentroCusto.findOne({
            fazenda_id: maintenance.asset.fazenda_id,
            ativo: true
        });

        if (maintenance.totalCost > 0) {
            await LancamentoCusto.create({
                fazenda_id: maintenance.asset.fazenda_id,
                centro_custo_id: centroCusto ? centroCusto._id : null,
                data: maintenance.endDate,
                descricao: `Manutenção ${maintenance.type}: ${maintenance.title} (${maintenance.asset.assetId})`,
                categoria: maintenance.type === 'corretiva' ? 'manutencao_maquinas' : 'pecas_reposicao',
                valor: maintenance.totalCost,
                origem: 'manutencao',
                origem_id: maintenance._id,
                company: req.user.company,
                criado_por: req.user.id
            });
        }

        // 4. Histórico do Ativo: Timeline e Status
        await Asset.findByIdAndUpdate(maintenance.asset._id, {
            status: 'ativo',
            lastMaintenanceDate: maintenance.endDate,
            nextMaintenanceDate: new Date(Date.now() + (maintenance.asset.maintenanceInterval || 180) * 24 * 60 * 60 * 1000)
        });

        await AssetTimeline.create({
            asset: maintenance.asset._id,
            eventType: 'manutencao',
            eventDate: maintenance.endDate,
            user: req.user.id,
            title: `Manutenção Concluída (ERP): ${maintenance.title}`,
            description: `Finalização automatizada. Custo Total: R$ ${maintenance.totalCost.toFixed(2)}`,
            relatedMaintenance: maintenance._id,
            cost: maintenance.totalCost,
            metadata: {
                partsUsed: maintenance.parts.map(p => p.name),
                laborCost: maintenance.laborCost
            }
        });

        // 5. Atualizar Ticket Relacionado se existir
        if (maintenance.relatedTicket) {
            await Ticket.findByIdAndUpdate(maintenance.relatedTicket, {
                status: 'fechado',
                resolvedAt: new Date()
            });
        }
        ...
        res.json({
            success: true,
            message: 'Manutenção finalizada e integrada ao ERP com sucesso',
            data: maintenance
        });
